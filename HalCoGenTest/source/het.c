...
/* USER CODE END */
    ...
    *     - Enable HET
    */
    hetREG1->GCR = (0x01000001U | (uint32)((uint32)1U << 16U) | (0x00020000));
...
void pwmSetDuty(hetRAMBASE_t * hetRAM, uint32 pwm, uint32 pwmDuty)
{
    ...
    hetRAM->Instruction[(pwm << 1U) + 41U].Data = (uint32)((pwmPeriod * pwmDuty) / (uint32)100U) + 128U;
}
...
void pwmSetSignal(hetRAMBASE_t * hetRAM, uint32 pwm, hetSIGNAL_t signal)
{
    ...
    if(hetRAM == hetRAM1)
    {
        temp = ((float32)signal.period * 1000.0F) / 800.000F;
